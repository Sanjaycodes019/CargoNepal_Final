...
const TruckFormModal = ({ isOpen, onClose, onSubmit, formData, onChange, editingTruck, isAdmin = false, onRemoveImage }) => {
  ...
  const removeImage = (e) => {
    ...
    // Call the parent removeImage function if provided
    if (onRemoveImage) {
      onRemoveImage();
    } else {
      // Fallback: Trigger change with no files to properly clear the image
      const event = { 
        target: { 
          name: "image", 
          type: "file", 
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 md:p-6">
      ...
      <div className="relative bg-white w-full h-full sm:h-auto sm:max-h-[92vh] sm:rounded-3xl sm:max-w-2xl overflow-hidden shadow-2xl flex flex-col animate-fadeIn">
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8 custom-scrollbar">
          <form id="truck-form" onSubmit={onSubmit} encType="multipart/form-data">
            ...
            {/* Row 5: Availability Toggle - Only show for owners, not admins */}
            {!isAdmin && (
            ...
            {/* Row 6: Verification Toggle - Only show for admin editing */}
            {isAdmin && editingTruck && (
              <div className="flex items-center justify-between bg-blue-50 p-4 rounded-xl border border-blue-200">
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-blue-900">Verification Status</span>
                  <span className="text-xs text-blue-500">Toggle to verify or unverify this truck</span>
                </div>
                <Toggle 
                  checked={formData.isVerified || false} 
                  onChange={(e) => {
                    onChange({ target: { name: 'isVerified', type: 'checkbox', checked: e.target.checked }})
                  }} 
                />
              </div>
            )}
